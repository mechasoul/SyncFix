...
{
    /// <summary>
    /// contains harmony patches used for implementing the new simple time sync logic. while generally less effective than
    /// the ggpo algorithm, this has the advantage of being able to fix host advantage even when not all players have the
    /// mod installed
    /// </summary>
    [HarmonyPatch]
    public class TimeSyncSimple_Patches
    {
        /*
         ...
         * time that players pause for is 0.75 * how far ahead they are. this is probably an "err on the side of caution"
         * thing and helps to avoid oscillating pauses and stuff since the current frame numbers are all estimates anyway,
         * but i think 0.75 is maybe a bit overly conservative so i'm gonna play with some other numbers. probably mostly
         ...
         * in a 2p game, it's pretty straightforward. if we're the host, then we delay our time sync messages to ourselves 
         * by approximately how long it would take for the client to receive our time sync messages. we also improve the 
         * synchronization of timed messages, fixing the asymmetry at match start. this should fix host advantage.
         * if we're the client, then we try to just emulate what the vanilla host is doing: we calculate our own time sync
         * based on our own estimates, and ignore any time sync messages from the host. we don't fix the asymmetry at match
         * start, unfortunately. TODO this is maybe fixable by adding delay to timed messages proportional to the erroneous 
        ...
        public static IEnumerable<CodeInstruction> AlignTimesTranspiler(IEnumerable<CodeInstruction> instructions, ILGenerator iLGenerator)
        {
            ...
            cm.MatchStartForward(new CodeMatch(OpCodes.Ldc_R4, 0.5f))
                .SetInstruction(new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(TimeSyncSimple_Patches), nameof(TimeSyncSimple_Patches.GetPingFactor))));
            ...
            cm.Insert(new InstructionBuilder()
                ...
                .OpCode(OpCodes.Ldloc_S).Operand((sbyte)8)
                .OpCode(OpCodes.Ldc_R4).Operand(SIMPLE_VANILLA_ALIGN_TIMES_FACTOR)
                .OpCode(OpCodes.Mul)
            ...
            cm.Insert(new InstructionBuilder()
                ...
                .OpCode(OpCodes.Call).Operand(AccessTools.Method(typeof(TimeSyncSimple_Patches), nameof(TimeSyncSimple_Patches.SendTimeAlignMessage)))
                .Build());
